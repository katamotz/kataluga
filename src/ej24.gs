...
// Copia el nombre de la imagen
class Ejercicio24:GLib.Object
	palabra: string
	...
	def inicio()
		...
		imagen= new Imagen (50,150, directorio_datos+"/fotos/"+tostring(4,datos.transforma_archivo_int(palabra) )+".png")
		...
		palabra_solucion=palabra.replace("_","")
		
		var texto_ayuda = new Texto (250,170,Mm(palabra_solucion))
		texto_ayuda.arrastrable=false
		texto_ayuda.set_tamanotexto(30)
		
		correccion= new Texto_color (400,330,Mm(palabra_solucion))
		correccion.set_colorletra(azul)
		correccion.lista_de_colores.clear()
		correccion.arrastrable=false
		correccion.visible=false
		
		entrada= new Entrada (300,450,250,50,"")
		entrada.set_maximo_caracteres(longitud(palabra)+5)
	...
	def on_corregir()
		if not finalizado
			if entrada.get_texto().up()==palabra_solucion.up()
				sonidos.play("ondo")
				datos.guardar_informe ("Bien"+" ---- "+fecha_str+" ---- Ejercicio: "+nomejer[ejercicio]+" ---- Solución: "+palabra_solucion+" ---- Respuesta:"+entrada.get_texto())
				...
				sonidos.play ("gaizki")
				datos.guardar_informe ("Mal "+" ---- "+fecha_str+" ---- Ejercicio: "+nomejer[ejercicio]+" ---- Solución: "+palabra_solucion+" ---- Respuesta:"+entrada.get_texto())
